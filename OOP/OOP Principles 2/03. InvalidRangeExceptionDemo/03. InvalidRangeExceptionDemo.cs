...
class InvalidRangeExceptionDemo
{
    ...
    private static int ReadInteger(int start, int end)
    {
        ...
        do
        {
            Console.Write("Enter integer between [{0}...{1}]: ", start, end);
        ...
        {
            throw new InvalidRangeException<int>("Input value is not in allowed range.", start, end);
        }
    ...
    private static DateTime ReadDate(DateTime start, DateTime end)
    {
        ...
        do
        {
            Console.Write("Enter a date between [{0} - {1}]: ", start.ToString(DATE_FORMAT), end.ToString(DATE_FORMAT));
            inputValue = Console.ReadLine();
        }
        ...
        {
            throw new InvalidRangeException<DateTime>("Input value is not in allowed range.", start, end);
        }
